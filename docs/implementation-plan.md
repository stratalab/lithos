...
**Hybrid-linear-attention interaction (noted 2026-07-17, on the Kimi K3 launch).** Kimi Linear (arXiv 2510.26692 — the published research behind K3; K3 itself shipped API-first with no tech report yet) validates a **3:1 hybrid** of Kimi Delta Attention (gated-DeltaNet linear attention, *fixed-size* recurrent state) with full MLA at 48B-A3B: ~75% KV-cache reduction, up to 6× decode throughput at 1M context, parity-or-better vs full attention *including RL scaling*. Consequence for R2: if the from-scratch line ever adopts a KDA-style hybrid (banked sweep arm — see the E10 architecture-decisions block in `post-training-implementation-plan.md`), the offload target shrinks ~75% but does **not** vanish — the interleaved full-attention layers' KV still grows with context. That is a *cleaner* R2 story, not a weaker one: the recurrent state is small by construction and stays GPU-resident; StrataDB offloads only the few full-attention layers' KV — a smaller, better-bounded working set, so long-context-on-edge gets easier to hit. It does soften the "KV exceeds weights" premise at the margin, so state the R2 pitch as **conditional on the attention architecture**: full-attention models = the big win; hybrid models = a smaller but still-real win on the remaining full layers. Watch for the K3 technical report ("Attention Residuals" has no paper behind it yet — if it's a cheap stabilizer for hybrid stacks, it matters at our scale too).
